...
describe('GameMechanics', () => {
  ...
  beforeEach(() => {
    ...
    mockUpgrades = [
      ...
      },
      {
        ...
        type: 'dpsMultiplier',
        purchased: 0,
        cost: () => 100 * Math.pow(1.15, mockUpgrades.find(u => u.id === 4).purchased),
        value: 1.15
      }
    ];
  ...
  describe('purchaseUpgrade', () => {
    ...
    it('should handle purchase errors gracefully', () => {
      ...
      mockUpgrades[0].cost = () => { throw new Error('Cost calc error'); };
      const result = mechanics.purchaseUpgrade(3);
      expect(result).toBe(false);
      ...
      expect(console.error).toHaveBeenCalledWith('Error in canAfford:', expect.any(Error));
    });
  });
  ...
  describe('canAfford', () => {
    ...
    it('should handle cost calculation errors', () => {
      const upgrade = { 
        cost: () => { throw new Error('Test error'); },
