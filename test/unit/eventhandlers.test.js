...
describe('EventHandlers', () => {
  ...
  it('should remove existing click listener before adding new one', () => {
    ...
    const firstHandler = doroImage.addEventListener.mock.calls
    ...
    expect(doroImage.removeEventListener).toHaveBeenCalledWith('click', firstHandler);
    
    // Verify new handler was added
    expect(doroImage.addEventListener).toHaveBeenCalledWith('click', expect.any(Function));
  });
  ...
  beforeEach(() => {
    ...
    jest.spyOn(DOMHelper, 'getSidebarElement').mockReturnValue(sidebarElement);
  });
  ...
  beforeEach(() => {
    ...
    const viewButtonListeners = eventHandlers._listeners.filter(
      l => l.element.classList.contains('view-button')
    );
