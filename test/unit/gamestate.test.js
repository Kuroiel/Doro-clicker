...
describe('GameState', () => {
  ...
  gameState.increment(NaN);
  expect(gameState.doros).toBe(originalDoros);
  expect(console.error).toHaveBeenCalledWith('Invalid increment amount:', NaN);
});
  });
  ...
  describe('Listener System', () => {
    ...
    it('should handle listener errors gracefully', () => {
      ...
      expect(console.error).toHaveBeenCalledWith('State listener error:', expect.any(Error));
    });
...
describe('Performance', () => {
  it('should handle large numbers of autoclickers efficiently', () => {
    const largeAutoclickers = Array(10000).fill().map((_, i) => ({
      value: i % 10,
      purchased: (i % 5) + 1
    }));
