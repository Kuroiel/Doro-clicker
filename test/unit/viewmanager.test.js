...
describe('ViewManager', () => {
  ...
  const mockViewButtons = [
    { dataset: { view: 'autoclickers' }, classList: { add: jest.fn(), remove: jest.fn() } },
    { dataset: { view: 'upgrades' }, classList: { add: jest.fn(), remove: jest.fn() } }
  ];

  const mockUpgradeViews = [
    { id: 'autoclickers-container', classList: { add: jest.fn(), remove: jest.fn() } },
    { id: 'upgrades-container', classList: { add: jest.fn(), remove: jest.fn() } }
  ];
  ...
  beforeEach(() => {
    ...
    DOMHelper.toggleClass.mockImplementation((element, className, condition) => {
  ...
  describe('switchView', () => {
    ...
    it('should not switch to an invalid view', () => {
      ...
      expect(consoleWarnSpy).toHaveBeenCalledWith('Attempted to switch to invalid view: invalid-view');
      consoleWarnSpy.mockRestore();
    });

    it('should handle null DOM elements gracefully', () => {
      DOMHelper.getViewButtons.mockReturnValue([null, { dataset: { view: 'upgrades' }, classList: { add: jest.fn(), remove: jest.fn() } }]);
      DOMHelper.getUpgradeViews.mockReturnValue([null, { id: 'upgrades-container', classList: { add: jest.fn(), remove: jest.fn() } }]);
  ...
  describe('_updateViewButtons', () => {
    it('should update button active states based on current view', () => {
      ...
      expect(mockViewButtons[0].classList.add).not.toHaveBeenCalledWith('active');
      expect(mockViewButtons[0].classList.remove).toHaveBeenCalledWith('active');
      expect(mockViewButtons[1].classList.add).toHaveBeenCalledWith('active');
      expect(mockViewButtons[1].classList.remove).not.toHaveBeenCalledWith('active');
    });
  ...
  describe('_updateViewContainers', () => {
    it('should update container visibility and call renderUpgrades for active view', () => {
      ...
      expect(mockUpgradeViews[0].classList.add).not.toHaveBeenCalledWith('active-view');
      expect(mockUpgradeViews[0].classList.remove).toHaveBeenCalledWith('active-view');
      expect(mockUpgradeViews[1].classList.add).toHaveBeenCalledWith('active-view');
      expect(mockUpgradeViews[1].classList.remove).not.toHaveBeenCalledWith('active-view');
